...
- **FR-004** *(option A, §3)*: When no map file exists at start-up, the daemon MUST write a minimal empty map **byte-identical** to the one `cuems-common` ships (200 bytes, an empty node list), and then load it through the same path used for an existing map. Byte-identical rather than merely "the same shape": the bytes are also the test fixture, so the two cannot drift, and a root element carrying no node-list element at all is a shape the library cannot refresh or save.
...
- **FR-006**: All other behaviour MUST stay as it is: the adopt/unadopt response contract with the UI (constitution IV), writing only when the map changed, retrying a failed write, tolerating a map that does not yet list this node, and failing loudly on an invalid map. **The end-to-end verification constitution IV requires for that chain is deliberately deferred** — it needs a controller and the operator UI. The deferral is recorded, not silent: FR-012 puts it on one checklist with every other verification owed.
...
- **FR-011** *(library prerequisite)*: The daemon MUST run against a library build in which loading a map that lists **no** nodes raises the same "this node is not in the map" error as a populated map that omits this node. Until `cuems-utils` `e363d03` it raised a different error type and the daemon crash-looped on every fresh install. **The version string cannot express this** — the fix shipped inside the pinned `0.1.0rc16` — so the suite MUST detect a build that predates it, rather than a node discovering it at boot.
- **FR-012** *(verification ledger)*: Every verification this feature cannot perform on a development checkout MUST be listed in **one** checklist, with what to do and what would prove it. That checklist MUST also carry feature 001's outstanding operator-chain verification (its **T050**) and, for the same reason, its sibling discovery check (**T051**), which is likewise unperformed — so the hardware debt of both features is in one place rather than spread across two features' records.
...
- **SC-007**: A stale library fails the test suite rather than a node at boot (FR-011).
- **SC-008**: One checklist lists every outstanding hardware or manual verification for this feature **and** feature 001's T050, each with its steps and its current state (FR-012).
...
- **The library must be a build at `cuems-utils` `e363d03` or later** (FR-011). That commit made an empty map raise the documented error instead of a `TypeError`; before it, this feature's own seed could not be read back. It shipped **inside** `0.1.0rc16`, so the pins do not move and the merge candidate is not re-cut — but `>= 0.1.0rc16` is also satisfied by a build that still crashes, so venvs and packaging runs must be rebuilt from the fixed checkout.
