...
- [ ] T049 [P] Report the two stale docstrings upstream in `cuems-utils` — `NodeIndex.set_controller_always_adopted` (`NodeList.py:177`) still says "on a first run, nothing else is", and `CuemsNetworkMapType.refresh` (`network_map.py:156-162`) still calls the un-ported branch an open item, when the consumer closed it by deleting the branch. **Report written** at `specs/001-network-map-object-adoption/upstream-report.md`, which also carries a third, more serious finding (the `0600` save mode — see T052). **Still open**: filing it into `cuems-utils` is an edit to another repository and belongs to whoever owns it. Report, do not patch — the yardstick's guarantee depends on that file not being edited from this side
