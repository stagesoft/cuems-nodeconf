...
- [ ] T035 [US2] Update the template path construction in `cuemsnodeconf/CuemsNodeConf.py` from `+ '.master'` / `+ '.slave'` to `+ '.controller'` / `+ '.node'` — **re-measured 2026-09-17 to `:404` and `:442`**; the plan's `:381`/`:419` moved when US1 shrank the file, so locate them by name (`_install_master_service_template`, and the inline copy in `set_node_role`) rather than by line (FR-016)
...
- [ ] T041 [US2] Cross-check the pinned vocabulary against `cuems-common`'s half — its feature `specs/001-node-role-and-conversion-ordering` (33 tasks, none started as of `e149089` on `feat/xml-refactor`) — key, all three values and all three filenames must agree exactly before either side merges. Its T004 cross-checks this repository's `contracts/avahi-txt.md` in return, so keep that file stable
- [ ] T042 [US2] 🚧 **MERGE GATE (FR-019) — blocking, do not clear early.** Confirm in `/home/stagelab/cuems-common` that `specs/001-node-role-and-conversion-ordering` has landed its US1 — the template renames (its T008/T009) **and** the consumers that name those files, `usr/bin/cuems-config-node:64` and `etc/sudoers.d/99-cuems:3-5` (its T007) — and is ready to merge in the same window as this one; record that branch and commit in this PR's description, and agree the window with flow 03's owner. **US2 and US3 do not merge until this task is checked.** A publisher and a listener disagreeing about the TXT key discover nothing, and the failure is silent — no error, no exception, nodes simply never appear. A rename that lands without those two consumers breaks provisioning instead
