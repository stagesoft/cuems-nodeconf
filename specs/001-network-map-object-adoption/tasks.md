...
- [ ] T042 [US2] 🚧 **MERGE GATE (FR-019) — blocking, do not clear early.** Confirm in `../cuems-common` that `specs/001-node-role-and-conversion-ordering` has landed its US1 — the template renames and the consumers naming those files — and is ready to merge in the same window as this one; record that branch and commit in this PR's description, and agree the window with flow 03's owner. **US2 and US3 do not merge until this task is checked.** A publisher and a listener disagreeing about the TXT key discover nothing, and the failure is silent. **Counterpart as of 2026-09-18**: **`f2fc0f5`** (`feat/xml-refactor`, its pushed tip) — the commit that becomes the merge-gate tag **`xml-refactor-merge-candidate`**, pinned so the commit this gate cites does not move when the gate-closing commits land. It supersedes `1a00159` by one commit that touches nothing under `debian/` (`git diff 1a00159..f2fc0f5 -- debian/` is empty), so the counterpart still carries the 1.3.0-23 changelog, `Breaks: cuems-nodeconf (<< 0.1.0-8)`, the renamed templates and `node_role=` in both TXT records — and T047's demonstration, built from `1a00159`, describes the same packages. That commit extends cuems-common's own demonstration to both directions, mirroring this repository's `debian/control` at `56f6123`, and records their merge target as `rc_1` (a fast-forward). Their gate T019 is still open, waiting on *this* repository being ready, which it is. What remains is the human agreement on the window — neither half merges alone
