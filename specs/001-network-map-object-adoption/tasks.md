...
- [ ] T047 [US3] Build the `.deb`s and demonstrate the out-of-order refusal per `quickstart.md` §4 (SC-007) — a gate that has never been demonstrated is a claim. **Not performable in this environment (checked 2026-09-17)**: `dh-virtualenv` is absent, so this package cannot be built here, and no `cuems-common` `.deb` exists to install against. Needs a Debian build sandbox with both packages built
